...
object SparkScalaBitcoinPageRank {
   def main(args: Array[String]): Unit = {
	...
	jobPageRank(sc,hadoopConf,args(0),args(1))
	sc.stop()
      }

 def jobPageRank(sc: SparkContext, hadoopConf: Configuration, inputFile: String, outputFile: String): Unit = {
val bitcoinBlocksRDD = sc.newAPIHadoopFile(inputFile, classOf[BitcoinBlockFileInputFormat], classOf[BytesWritable], classOf[BitcoinBlock],hadoopConf)
	...
	pageRank.saveAsTextFile(outputFile)
}
